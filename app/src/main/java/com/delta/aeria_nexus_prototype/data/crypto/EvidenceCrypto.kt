 ...
 * la de la unidad bodycam (BodyCamServer). Este fichero es un port literal del suyo:
 * las dos apps tienen que producir exactamente el mismo formato, byte a byte, para
 * que la evidencia se analice igual venga de donde venga.
 *
 * ── Por qué se cifra por bloques ──────────────────────────────────────────────
 *
 * En Android AES/GCM **no se puede cifrar en streaming**: Conscrypt acumula todo lo
 * que se le pasa por Cipher.update() y solo cifra en doFinal(). Verificado en la
 * unidad bodycam, pedirle 256 MB de una vez revienta con OutOfMemoryError en
 * OpenSSLCipher$EVP_AEAD.expand. Es inherente a los modos AEAD: el tag cubre todo el
 * mensaje, así que no hay salida parcial que emitir hasta el final. El teléfono corre
 * el mismo Conscrypt, así que el razonamiento vale igual aquí.
 *
 ...
 * ── Cuándo se ejecuta ─────────────────────────────────────────────────────────
 *
 * Siempre **al cerrar** la captura, nunca en paralelo a la grabación: el cifrado
 * compite por CPU e I/O con el grabador. Coste medido en la unidad bodycam sobre
 * grabación real de 20 min (473 MB): ~5,1 s.
 */
object EvidenceCrypto {
    ...
    /**
     * Si el fichero en claro se borra tras cifrarlo.
     *
     * En `true` desde que existe la bóveda del agente (EVD-007 resuelto): la captura
     * ya no se queda en la galería, y lo único que sobrevive en el dispositivo es el
     * .fev. Quien quiera volver a verla desbloquea la bóveda con su contraseña.
     */
    const val DELETE_PLAINTEXT = true
    ...
    data class Sealed(
        val file: File,
        /** SHA-256 del fichero original: el de la cadena de custodia. */
